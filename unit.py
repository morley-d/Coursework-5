...
class BaseUnit(ABC):
    ...
    def _count_damage(self, target: BaseUnit) -> int:
        ...
        if target.stamina > target.armor.stamina_per_turn:
            target_armour = target.armor.defence * target.unit_class.armor
            target.stamina -= target.armor.stamina_per_turn
        else:
            target_armour = 0
        damage = unit_damage - target_armour
    ...
    def add_stamina(self, stamina_point):
        stamina_grouth = stamina_point * self.unit_class.stamina
        if self.stamina + stamina_grouth > self.unit_class.max_stamina:
            self.stamina = self.unit_class.max_stamina
        else:
            self.stamina += stamina_grouth
